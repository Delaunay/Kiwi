...
inline const std::string &name(BuiltinType id) {
...
template <typename T> BuiltinType type_id() { return BuiltinType::object; }
template <> inline BuiltinType type_id<typetype>() { return BuiltinType::type; }

#define X(n)                                                                                       \
    template <> inline BuiltinType type_id<n>() { return BuiltinType::n; }
KIWI_TYPE_TYPE
#undef X

template <typename T, typename NodeTrait> Expression<NodeTrait> *type() {
    static generic::Type<Expression<NodeTrait>> t(name(type_id<T>()));
    return &t;
}

template <typename NodeTrait> class Value : public Expression<NodeTrait> {
  public:
    NODE_TYPES;

    template <typename T>
    Value(T x) :
        Expression<NodeTrait>(NodeTag::value), _self(std::make_shared<Model<T>>(std::move(x))),
        type(kiwi::type_id<T>()) {}
...
template <typename T, typename NodeTrait> const T &as(Value<NodeTrait> *expr) {
    return expr->as(T());
}
