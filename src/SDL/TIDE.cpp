...
public:
    ...
    Point draw(SDL_Renderer *renderer, Point position) override {
        if (!_drawable_cache) {
            ...
            _drawable_cache = TexturePtr{ SDL_CreateTexture(renderer, SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_TARGET, 1, 1) };
            ...
            // Create Real target
            _drawable_cache = TexturePtr{ SDL_CreateTexture(renderer, SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_TARGET, _size.x, _size.y) };
            SDL_SetRenderTarget(renderer, _drawable_cache.get());
...
public:
    ...
    void insert_entity(Drawable* d) {
        _entities.push_back(d);
    }
