...
class WindowManager {
  ...
  public:
    ...
    template <typename T>
    static T *new_window(const String &str, i32 x, i32 y, i32 w, i32 h,
                         u32 flag = SDL_WINDOW_SHOWN | SDL_WINDOW_RESIZABLE) {
        return WindowManager::manager().make_window<T>(str, x, y, w, h, flag);
    }

    template <typename T>
    static T *new_window(const String &str, i32 w, i32 h,
                         u32 flag = SDL_WINDOW_SHOWN | SDL_WINDOW_RESIZABLE) {
        return WindowManager::manager().make_window<T>(str, SDL_WINDOWPOS_UNDEFINED,
                                                       SDL_WINDOWPOS_UNDEFINED, w, h, flag);
    }

    void window_exposed(u32 id) { log_info("exposed ", id); }
    ...
    void window_shown(u32 id) { log_info("shown ", id); }
    void window_hidden(u32 id) { log_info("hidden ", id); }

    void window_moved(u32 id) { log_info("moved ", id); }
    void window_resized(u32 id) { log_info("resized ", id); }
    void window_size_changed(u32 id) { log_info("size_changed ", id); }

    void window_maximized(u32 id) { log_info("maximized ", id); }
    void window_minimized(u32 id) { log_info("minimized ", id); }
    void window_restored(u32 id) { log_info("restored ", id); }
    ...
    void window_take_focus(u32 id) { log_info("take_focus ", id); }
    ...
    void handle_window_event(SDL_WindowEvent event) {
        ...
        case SDL_WINDOWEVENT_EXPOSED:
            return window_exposed(id);

        case SDL_WINDOWEVENT_ENTER:
            return window_mouse_enter(id);
        case SDL_WINDOWEVENT_LEAVE:
            return window_mouse_leave(id);

        case SDL_WINDOWEVENT_FOCUS_GAINED:
            return window_gained_focus(id);
        case SDL_WINDOWEVENT_FOCUS_LOST:
            return window_lost_focus(id);

        case SDL_WINDOWEVENT_SHOWN:
            return window_shown(id);
        case SDL_WINDOWEVENT_HIDDEN:
            return window_hidden(id);

        case SDL_WINDOWEVENT_MOVED:
            return window_moved(id);

        case SDL_WINDOWEVENT_RESIZED:
            return window_resized(id);
        case SDL_WINDOWEVENT_SIZE_CHANGED:
            return window_size_changed(id);

        case SDL_WINDOWEVENT_MAXIMIZED:
            return window_maximized(id);
        case SDL_WINDOWEVENT_MINIMIZED:
            return window_minimized(id);
        case SDL_WINDOWEVENT_RESTORED:
            return window_restored(id);

        case SDL_WINDOWEVENT_CLOSE:
            return window_close(id);
        case SDL_WINDOWEVENT_TAKE_FOCUS:
            return window_take_focus(id);
    ...
    template <typename T> T *push_back(SDL_Window *win) {
  ...
  private:
    template <typename T> T *make_window(const String &str, i32 x, i32 y, i32 w, i32 h, u32 flag) {
