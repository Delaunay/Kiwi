...
template <typename T> T *check(T *ptr, const char *msg, const char *file, int line) {
    if(ptr == nullptr) {
...
template <typename T, typename D> T *get_ref(const std::unique_ptr<T, D> &ptr) { return ptr.get(); }

inline void set_pixel(SDL_Surface *surface, i32 x, i32 y, SDL_Color color) {
...
inline void render_surface(SDL_Renderer *renderer, i32 x, i32 y, SDL_Surface *surface, i32 w = -1,
                           i32 h = -1) {
    SDL_Rect box{x, y, w == -1 ? surface->w : w, h == -1 ? surface->h : h};
    TexturePtr hexT{SDL_CreateTextureFromSurface(renderer, surface)};
    SDL_RenderCopy(renderer, hexT.get(), nullptr, &box);
}

struct SurfaceLock {
    SurfaceLock(SDL_Surface *s) : _surface(s) { SDL_LockSurface(_surface); }

    ~SurfaceLock() { SDL_UnlockSurface(_surface); }
