...
class FunctionDrawable: public ExpressionDrawable{
    public:
        ...
        template<typename T>
        FunctionDrawable(Function<T> const* fun, uint32 idt = 0){
            Point pos{0, 0};
            ...
            //
            size_t n = size_t(i32(fun->args_size()) - 1);
            ...
            if (n >= 0){
                ...
                for(size_t i = 0; i < n; ++i){
                    ...
                    std::tie(std::ignore, pos) = insert_entity_after(make_string(": "), pos);
                    std::tie(types[i],    pos) = insert_entity_after(ExpressionDrawable::make(fun->type->arg(i)), pos);
                    std::tie(std::ignore, pos) = insert_entity_after(make_string(", "), pos);
                }
                ...
                std::tie(types[n], pos) = insert_entity_after(ExpressionDrawable::make(fun->type->arg(n)), pos);
            }

            std::tie(std::ignore, pos) = insert_entity_after(make_string(")"), pos);
            std::tie(std::ignore, pos) = insert_entity_after(make_string(style.arrow()), pos);

            std::tie(return_type, pos) = insert_entity_after(ExpressionDrawable::make(fun->type->return_type), pos);
            std::tie(std::ignore, pos) = insert_entity_after(make_string(": "), pos);

            // I could make both of those dummy Drawables to ease size computation when changes are registered
            pos = new_line(pos);
