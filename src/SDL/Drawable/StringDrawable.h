...
class StringDrawable : public Drawable {
  public:
    StringDrawable(const std::string &str, SDL_Color color = default_color(),
                   SDL_Font *font = default_font()) :
        _string(str),
    ...
    Point off_screen_draw(SDL_Renderer *renderer, Point position) {
        ...
        if(!_texture) {
            ...
            if(_string.size() > 0) {
                auto text =
                    TexturePtr{CHECK(SDL_CreateTextureFromSurface(renderer, _surface.get()))};
    ...
    SDL_Surface *render_utf8(RenderingType kind, const String &txt, SDL_Font *font, SDL_Color color,
        ...
        case RenderingType::Blended:
            return CHECK(TTF_RenderUTF8_Blended(font, txt.c_str(), color));
        case RenderingType::Shaded:
            return CHECK(TTF_RenderUTF8_Shaded(font, txt.c_str(), color, color_bg));
        }
        return CHECK(TTF_RenderUTF8_Solid(font, txt.c_str(), color));
    }
