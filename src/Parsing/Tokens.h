...
class Token {
  public:
    Token(TokenType t, uint32 l, uint32 c) : _type(t), _line(l), _col(c) {}

    Token(int8 t, uint32 l, uint32 c) : _type(t), _line(l), _col(c) {}
    ...
    operator bool() { return _type != tok_eof; }
...
inline std::ostream &operator<<(std::ostream &out, class Token &tok) { return tok.print(out); }
